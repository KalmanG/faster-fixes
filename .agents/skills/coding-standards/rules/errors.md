...
How errors surface to the user. Implements `docs/adr/0010-domain-errors-and-transport-mapping.md`.
...
Client code never imports `@/server/errors/*` and never relies on `instanceof DomainError` (it does not survive serialization). Branch on the transported code instead: `error.data.code` (tRPC) or `result.serverError.code` (safe-action).

## The four display channels

- **Mutations (tRPC mutation / server action): toast.** Read the message from the failed mutation/action and show it via the Sonner toast. The message is already final French copy for `DomainError`s; `INTERNAL_SERVER_ERROR` arrives pre-masked as "Une erreur est survenue, réessaie plus tard."
...
- A boundary must never render `error.message`: Server Component errors carry a masked digest, and an unexpected message may leak internals. Log the raw error in `useEffect`; show fixed French copy to the user.
